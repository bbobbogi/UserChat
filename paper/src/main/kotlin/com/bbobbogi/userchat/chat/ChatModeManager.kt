...
class ChatModeManager(private val config: UserChatConfig) {
    private val playerModes = ConcurrentHashMap<UUID, ChatMode>()

    fun getMode(playerUuid: UUID): ChatMode {
        return playerModes[playerUuid] ?: config.defaultChatMode
    }

    fun setMode(playerUuid: UUID, mode: ChatMode) {
